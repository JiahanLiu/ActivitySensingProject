...
def create_time_series(xtrn, ytrn, xtst, ytst):
    ...
    scaled_testSet = scaler.fit_transform(testSet)
    scaled_YTrain = scaler.fit_transform(ytrn)
    scaled_YTest = scaler.fit_transform(ytst)

    #creates a (271088, 277 complete time series)
    time_series_train = series_to_supervised(scaled_trainSet, scaled_YTrain, 1, 1);
    time_series_test = series_to_supervised(scaled_testSet, scaled_YTest, 1, 1);
