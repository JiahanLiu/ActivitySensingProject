...
def train_model(X_train,Y_train,M_train,feat_sensor_names,label_names,sensors_to_use,target_label):
    ...
    lr_model.compile(optimizer=tf.train.AdamOptimizer(), 
              loss='sparse_categorical_crossentropy',
              metrics=['accuracy'])
    lr_model.fit(X_train,y,epochs=5);
...
def create_baseline():
    ...
    model.add(keras.layers.Dense(60, input_dim=225, kernel_initializer='normal', activation='relu'))
